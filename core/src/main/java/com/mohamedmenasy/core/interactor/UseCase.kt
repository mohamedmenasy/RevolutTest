...
/**
 * Base class for the integrator, which using the RxJava to handle it's operations
 * @param out Type
 * @param in Params
...
abstract class UseCase<out Type, in Params> where Type : Any {
  ...
  abstract class RxSingle<T, in P> : UseCase<Single<T>, P>() {
    fun execute(
        observer: UseCaseObserver.RxSingle<T>,
        params: P? = null
    ) =
  ...
  abstract class RxObservable<T, in P> : UseCase<Observable<T>, P>() {
    fun execute(
        observer: UseCaseObserver.RxObservable<T>,
        params: P? = null
    ) =
        disposables.add(build(params).subscribeWith(observer))
  ...
  abstract class RxFlowable<T, in P> : UseCase<Flowable<T>, P>() {
    fun execute(
        subscriber: UseCaseObserver.RxFlowable<T>,
        params: P? = null
    ) =
